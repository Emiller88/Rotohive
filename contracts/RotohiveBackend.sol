...
contract RotohiveBackend is StoppableShareable, RotohiveShared {
    ...
    string public name = "Rotohive";
    string public symbol = "NMR";
    uint256 public decimals = 18;

    event DelegateChanged(address oldAddress, address newAddress);

    function RotohiveBackend(address[] _owners, uint256 _num_required, uint256 _initial_disbursement) StoppableShareable(_owners, _num_required) {
        totalSupply = 0;
        total_minted = 0;
    ...
    function claimTokens(address _token) onlyOwner {
        ...
        RotohiveBackend token = RotohiveBackend(_token);
    ...
    function rotoTransfer(address _to, uint256 _value) onlyPayloadSize(2) returns(bool ok) {
        return delegateContract.delegatecall(bytes4(sha3("rotoTransfer(address,uint256)")), _to, _value);
    }
    ...
    function transferFrom(address _from, address _to, uint256 _value) stopInEmergency onlyPayloadSize(3) returns (bool ok) {
        require(!isOwner(_from) && _from != roto); // Transfering from Roto can only be done with the rotoTransfer function
