...
import "contracts/RotohiveShared.sol";

// Whoever creates the contract has the power to stop it, this person can be changed via transferOwnership(_new_address)
contract RotohiveDelegate is StoppableShareable, RotohiveShared {

    function RotohiveDelegate(address[] _owners, uint256 _num_required) StoppableShareable(_owners, _num_required) {
    }

    // All minted NMR are initially sent to Roto, obeying both weekly and total supply caps
    function mint(uint256 _value) onlyOwner returns (bool ok) {
        ...
        balanceOf[roto] = safeAdd(balanceOf[roto], _value);
    ...
    // Roto calls this function to release staked tokens when the staked predictions were successful
    ...
    // Anyone but Roto can stake on themselves
    ...
    // Only Roto can stake on behalf of other accounts. _stake_owner will always be Roto's hot wallet
    ...
    function _stake(address _staker, uint256 _value, bytes32 _tag, uint256 _tournamentID, uint256 _roundID, uint256 _confidence) stopInEmergency internal returns (bool ok) {
        ...
        require(!isOwner(_staker) && _staker != roto); // Roto cannot stake on itself
    ...
    // Transfer NMR from Roto account using multisig
    function rotoTransfer(address _to, uint256 _value) onlyManyOwners(sha3(msg.data)) returns (bool ok) {
        // Check for sufficient funds.
        require(balanceOf[roto] >= _value);

        balanceOf[roto] = safeSubtract(balanceOf[roto], _value);
        balanceOf[_to] = safeAdd(balanceOf[_to], _value);

        // Notify anyone listening.
        Transfer(roto, _to, _value);

        return true;
    }

    // Allows Roto to withdraw on behalf of a data scientist some NMR that they've deposited into a pre-assigned address
    // Roto will assign these addresses on its website
